...
export const getNAV = {
  definition: {
    ...
    description:
      "Get total net worth (NAV) in specified currency. Supports USD, EUR, CAD, AED, CHF, SGD. Costs 1 credit per address.",
    inputSchema: {
      type: "object",
      properties: {
        addresses: {
  ...
  async execute(args: any, apiClient: OctavAPIClient) {
    ...
    const data = await apiClient.getNAV(
      validated.addresses,
      validated.currency,
    );
